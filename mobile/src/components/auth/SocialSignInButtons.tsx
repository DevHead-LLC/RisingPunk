...
export const SocialSignInButtons = memo(function SocialSignInButtons({ 
  ...
  const handleAppleSignIn = useCallback(async () => {
    ...
    try {
      ...
      // Error codes:
      // 1000: User cancelled during credential entry or unknown error
      // 1001: User explicitly cancelled the authorization
      const isUserCancellation = errorCode === '1000' || 
                                errorCode === '1001' || 
      ...
      if (isUserCancellation) {
        ...
        // This includes credential entry cancellations (1000) and explicit cancellations (1001)
        return;
      }
      
      // Log error details for debugging (only for non-cancellation errors)
