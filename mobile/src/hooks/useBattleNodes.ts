...
interface GetInitialNodesParams {
  width: number;
  height: number;
  topMargin?: number; // Optional, default to 125
}
...
export function useInitialBattleNodes({ width, height, topMargin = 125 }: GetInitialNodesParams) {
  return useMemo(() => calculateNodePositions(width, height, topMargin), [width, height, topMargin]);
}
