 ...
 * Set `APPLE_ROOT_CA_PATHS` to a colon-separated list of absolute or repo-relative paths to `.cer` / `.pem` files
...
export function loadAppleRootCertificateBuffers(): Buffer[] {
  ...
  if (!raw || !raw.trim()) {
    throw new Error(
      'APPLE_ROOT_CA_PATHS is not set. Download Apple Root CA G3 (and related) .cer files and list colon-separated paths.',
    );
  }
  const paths = raw
    .split(':')
