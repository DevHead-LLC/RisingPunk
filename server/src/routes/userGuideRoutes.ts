...
router.get('/current-task', auth, async (req: Request, res: Response) => {
  try {
    ...
    ).select('completedTasks collectedTasks skippedTasks showTaskGuide profileVisitedAt themeChangedToDarkAt themeChangedToLightAt avatarChangedAt homeVisitedAt taskGuideShownAt').lean();
